...
#     roc_score = roc_auc_score(labels_all, preds_all) #计算分数
#     ap_score = average_precision_score(labels_all, preds_all) #计算精度
#
#     return roc_score, ap_score
#
#
# def load_data_microbe():
#     fname='mircobe-disease.txt'
#     disease_microbe_dic = {}
#     fin = open(fname)
#     next(fin)
...
#     fin.readline()
#     for line in fin:
#         disease, microbe, position, evidence,PMID = line.strip().split('\t')
#         edges.append((disease,microbe))
#         disease_set.add(disease)
#         microbe_set.add(microbe)
#     fin.close()
#     g = nx.Graph()
#     g.add_edges_from(edges)
#     adj = nx.adjacency_matrix(g)
#     print('disease',len(disease_set),'microbe',len(microbe_set))
#     return adj
#
def load_data(file):
...
#     coords = np.vstack((sparse_mx.row, sparse_mx.col)).transpose() #https://blog.csdn.net/csdn15698845876/article/details/73380803 #位置
#
...
#
#
adj = load_data("microRNA-disease.txt")
...
# adj_orig = adj - sp.dia_matrix((adj.diagonal()[np.newaxis, :], [0]), shape=adj.shape) #todo  把临近矩阵的对角线1元素去掉
...
#
# du.get_new_scoring_matrices('input.csv')
